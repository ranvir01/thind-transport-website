...
- **DAT: yes, a 15-truck carrier qualifies for the REST API.** Any load-board subscription tier allows a RESTful integration; you need a free service account + a Connexion seat + load-board seat on the acting user (~$149–$199/mo tier + seat costs, ~$500–1,000 one-time setup fee per third-party guides). Request via developersupport@dat.com with your MC number. RateView **API** is a separate, pricier gate (RateView Combo Pro/Premium) — skip it. [1][3][5][6]
- **Truckstop: yes, with paperwork.** Top-tier plan (~$110–$175/mo, tier naming shifted in 2026) + signed Systems Integration Agreement via tsi@truckstop.com gets a 6-digit Integration ID for the SOAP Load Search service — LoadOff's adapter already speaks this protocol (repo-verified). [15][16][18]
...
- **Cheapest legitimate lane rates: SONAR Quick Rates, $24.99/mo, self-serve, no contract** — unlimited TRAC spot-rate lane searches (van/reefer/flatbed, confidence score, low/high, 7/28-day trend) + market-tightness indicators. Sign up same-day at gosonar.com. Screen-only (no API). [56][57]
...
- **API door — the headline finding:** CHR advertises **free API connectivity for carriers**: *"find, offer, book, and auto-create C.H. Robinson loads directly in their own TMS,"* send visibility updates, upload documents, check payment status. The Carrier Connectivity page routes to a "Request API project" flow. This is a top-3 US broker handing a small carrier exactly the integration LoadOff is built for, at $0. (search-verified 2026-08-08 [40])
...
| **DAT One Enhanced / Pro** (in-app) | $99 / $149/mo | 15-day (Enhanced) or 7-day (Pro) average lane rates inside the board you'd buy anyway [6][7] | No (in-app; RateView *API* gated separately) | The default if you're buying DAT anyway |
...
| **DAT** | DAT One board, tiers $49–$199/mo; rates at Enhanced+ | **REST Load Board search/post API** via service account; BookNow beta; RateView API gated higher | ~$149–$199/mo tier + Connexion seat + ~$500–1,000 one-time (3rd-party fig.) | Any DAT subscription; Connexion + load-board seat on acting user; app certification; MC number | developersupport@dat.com; developer.dat.com | **2** |
