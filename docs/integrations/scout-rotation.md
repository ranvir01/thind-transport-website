...
Next up by this rule: after the 2026-07-25 `truckstop` re-pass, the oldest built-adapter doc is
`factor` (2026-07-21) — **prefer `factor` next** unless a human has since made TruckerCloud
contact (`truckercloud`, 2026-07-20, has hit a total 403 wall for five straight passes; anonymous
scouting there is exhausted). `truckstop` no longer needs an immediate re-pass: the 2026-07-25
...
before another docs pass would add anything. (Prior note, still true: the 2026-07-24 `efs` pass
