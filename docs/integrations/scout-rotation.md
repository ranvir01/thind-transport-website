...
| `mailbox` | **Built** — generic IMAP client, not a vendor SDK. Three auth paths live since 2026-07-11: Gmail app password, M365 client-credentials OAuth2, Google Workspace service-account OAuth2 (XOAUTH2). Cron is daily 12:30 UTC, not hourly as older notes said. 2026-07-27 pass (2nd re-scout): no adapter-breaking change — all three auth paths, cron cadence, and attachment/message caps verified against current `mailbox.ts`/`mailbox-oauth.ts` code, byte-for-byte unchanged. Two refining finds: (1) resolved the 2026-07-22 "SMTP AUTH retirement dates still in flux" note — found the primary Microsoft Community Hub post behind it (an updated timeline, ~Jan 27 2026): the March–April 2026 figure was the original plan, Dec 2026 (disabled by default for existing tenants) / H2 2027 (final removal announced) is the revised one; still SMTP-only, moot for this IMAP-only adapter; (2) named the previously-unpublished IMAP throttling knob — Exchange's `ImapMaxConcurrency` parameter (`Set-ThrottlingPolicy`/`Set-CASMailbox`), though its Exchange Online tenant-default number is still not published. Also flagged, not acted on: two getmailbird.com articles' search-synthesized summaries claimed Workspace app passwords are fully disabled, contradicting the 2026-07-22 five-source finding — direct fetch 403'd both times, so weighed as unconfirmed against six converging sources saying app passwords still work | `user`, `password` (Gmail only), `tenantId`/`clientId`/`clientSecret` (M365), `serviceAccountKey` (Workspace), `host`, `port`, `folder` | `src/lib/hub/mailbox.ts` + `mailbox-oauth.ts` | [`mailbox.md`](./mailbox.md) | 2026-07-27 |
...
| `dat` | **Built** — search + posting-to-load-draft mapper; registry `status: "live"` (manual sync). 2026-07-28 pass (4th): no adapter-breaking change — two-level org+user token auth, RateView-Combo scoping, and the ~28-min token cache all re-confirmed with no contradicting source. Two refining finds: (1) DAT's own Book Now sales page confirms **BookNow is still in beta as a TMS integration**, not GA — sharpens the standing "future booking slice" note; (2) a search snippet on the *posting* (broker-side, `POST posting/v2/loads`) endpoint shows a typed `contactMethods` array (`PRIMARY_PHONE`/`EMAIL`) instead of a flat phone field — single-source, wrong side of the API, page 403'd, flagged as an open question for `normalizeDatPosting`'s `contactPhone` assumption, not acted on. Pricing refined (docs-only): named carrier tiers **Standard ~$49/mo, Enhanced ~$99/mo, Pro ~$149/mo**, narrower than the prior $50–$300 range (the $300 end is shipper/broker pricing). Every DAT page (`one.support.dat.com`, `developer.dat.com`, `learn.tai-software.com`, `cloud.comms.dat.com`, two TMS-vendor DAT pages) still 403s this env — search-excerpt only | `serviceAccountEmail`, `password`, `actingUserEmail` (added 2026-07-20) | `src/lib/hub/integrations/dat.ts` | [`dat.md`](./dat.md) | 2026-07-28 |
...
| `wex` | **Built** — adapter shipped; real feed confirmed daily SFTP CSV (same as EFS) — signed file-drop webhook shipped 2026-07-18 (`processWexEvent`). 2026-07-28 pass (2nd re-pass): no adapter-breaking change (file-drop path + `normalizeWexRecord` untouched). Four finds, none urgent: (1) the 2026-07-22 account-number conflict now **tilts to 19-digit / `690046`-prefix** (`XXXX-00-XXXXXX-X` form corroborated from a second angle; Fleetio's 13-digit claim looks like a card-family/legacy format) — docs-only, adapter never parses the field, `creds-shopping-list.md` still says "13-digit" → integrations-lane Backlog; (2) the **NCR Voyix × WEX** partnership (2025-10-20, launch 2026) is a **merchant-acceptance red herring** — Voyix Connect gateway accepting WEX cards at 18k+ station POS, NOT the carrier feed (same class as comdata's NCR Voyix note); (3) FleetRabbit documents a **real-time partner-tier WEX API** delivering Level III data (no CSV/batch) and WEX Payments API is **OAuth** — sharpens the "register as WEX partner" open question (payoff = real-time push), still partner-gated, not carrier self-serve; (4) 2026 pricing firmed to ~$50 setup / ~$4→$2 per-card/mo by tier, FlexCard no-fee. Every WEX/Corpay page still 403s this env — search-excerpt only. Prior 2026-07-22 finds (2–4-day SFTP turnaround, selectable file frequency) still stand | `feedUser`, `feedPassword`, `webhookSecret` | `src/lib/hub/integrations/wex.ts` | [`wex.md`](./wex.md) | 2026-07-28 |
...
Next up by this rule: after the 2026-07-28 `dat` pass, the oldest non-exhausted built-adapter
docs are a three-way tie at 2026-07-23 — `terminal`, `fmcsa`, and `eia`. `truckercloud`
(2026-07-20) is older still but anonymous scouting there is exhausted (five straight total 403
walls) — skip until a human makes vendor contact. Recommend `terminal` next: it's the
highest production-impact of the three (live daily TSP-aggregator cron vs. the two free/adjacent
government APIs), and it's been four scout passes since its last genuinely new find (the
vehicle-location-change webhook, 2026-07-23). (`dat` moved to 2026-07-28.) `mailbox` stays the
least-walled provider in the rotation (its IMAP/OAuth research draws on Google/Microsoft
secondary sources that don't 403 the way the vendor-controlled fuel/board portals do) and is
where a future pass has the best odds of surfacing something real — this pass again found two
genuine refinements (SMTP AUTH timeline resolved, IMAP throttling knob named) rather than
logging a flat wall, unlike most of the vendor-portal providers below. Worth flagging for
whoever reviews this rotation: the **fuel-card + load-board cluster** (`comdata` now 6,
`truckercloud` 5, `efs` 3,
...
The 2026-07-28 `dat` pass (4th) found no adapter-breaking change: the two-level
service-account/acting-user auth, the RateView-Combo search/post-vs-rate-request split, and the
~28-minute token cache all re-confirmed with no contradicting source across four independent
search queries. Two refining, non-breaking finds: DAT's own Book Now sales page
(`cloud.comms.dat.com`, search-excerpt only) confirms BookNow is still in **beta** as a TMS
integration, not GA yet, sharpening the future-booking-slice note; and a single search snippet
on the *posting* (broker POST) side of the API shows a typed `contactMethods` array
(`PRIMARY_PHONE`/`EMAIL`) rather than a flat phone string, which — if the *search* side follows
the same convention — would mean `normalizeDatPosting`'s flat `contactPhone` assumption is wrong,
but this is single-source, on the wrong side of the API, and the page 403'd, so it's an open
question, not a change. Pricing narrowed to named carrier tiers (Standard ~$49/mo, Enhanced
~$99/mo, Pro ~$149/mo) from third-party 2026 load-board review sites, tightening but not
contradicting the existing $50–$300 range. Every DAT-controlled page and two TMS-vendor DAT
integration pages (`ftm.cloud`, `ascendtms.kayako.com`) 403'd this env again — search-excerpt
confirmation only, same wall as every other provider in this rotation.
