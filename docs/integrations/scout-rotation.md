...
Next up by this rule: after the 2026-07-26 `factor` re-pass, the oldest built-adapter doc is
`efs` (2026-07-20) — **prefer `efs` next** unless a human has since made TruckerCloud contact
(`truckercloud`, also 2026-07-20, has hit a total 403 wall for five straight passes; anonymous
scouting there is exhausted, so `efs` wins the tie). `truckstop` no longer needs an immediate re-pass: the 2026-07-25
