...
Next up by this rule: after the 2026-07-23 `fmcsa` + `eia` pass (below), the three oldest docs are
now `truckercloud` / `dat` / `efs`, all at 2026-07-20 — take the built adapter with the most
downstream impact first (all three are "Built"; no ranking beyond date needed since they tie).
