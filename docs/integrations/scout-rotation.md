...
| `fmcsa` (adjacent, free, not in `IntegrationProvider` union — no stored creds) | **Built** — QCMobile broker vetting, only docket/DOT lookups used (name/cargo/OOS/BASICS unused), no documented rate limit | `FMCSA_WEBKEY` env | `src/lib/hub/vetting.ts` | [`fmcsa.md`](./fmcsa.md) | 2026-07-07 |
...
"Adapter file" column). `fmcsa.md` is now researched (this cycle); the only remaining gap is
`eia.md`, still "missing — never researched."
...
Next up by this rule: `eia` (built, still missing its doc) — then back to the oldest-dated
vendor doc (`terminal.md`/`truckercloud.md`, both 2026-07-06).
