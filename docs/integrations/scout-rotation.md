...
Next up by this rule: after the 2026-07-25 `qbo` pass, the oldest built-adapter docs are
`truckercloud` (2026-07-20) and then `factor`/`truckstop` (both 2026-07-21). `truckercloud` has
hit a total 403 wall for five straight passes (anonymous scouting exhausted — only human outreach
can confirm its OAuth token endpoint + feed shape), so another anonymous pass yields nothing new;
**prefer `factor` or `truckstop`** unless a human has since made TruckerCloud contact. `truckstop`
still has the concrete open lead of confirming the `GetLoadSearchResults` response element names
(SOAP/XML, portal 403-walled), which makes it the more productive of the two the moment a
developer packet or non-403 source appears. (Prior note, still true: the 2026-07-24 `efs` pass
found no adapter-breaking change and did not re-corroborate the Motive "discounts applied" field.) The 2026-07-24 `dat` pass (3rd) found no adapter-breaking change
