...
Next up by this rule: after the 2026-07-24 `dat` pass, the two oldest docs are now
`truckercloud` and `efs`, both at 2026-07-20. `truckercloud` has hit a total 403 wall for five
straight passes (anonymous scouting exhausted — only human outreach can confirm its OAuth token
endpoint + feed shape), so a sixth anonymous pass yields nothing new; **prefer `efs`** (built
file-drop webhook adapter, real transaction-field research still productive) unless a human has
since made TruckerCloud contact. The 2026-07-24 `dat` pass (3rd) found no adapter-breaking change
