...
Next up by this rule: after the 2026-07-28 `wex` re-pass, the oldest non-exhausted built-adapter
doc is `dat` (2026-07-24) — take `dat` next. `truckercloud` (2026-07-20) is older still but
anonymous scouting there is exhausted (five straight total 403 walls) — skip until a human makes
vendor contact. (`wex` moved to 2026-07-28.) `mailbox` stays the
