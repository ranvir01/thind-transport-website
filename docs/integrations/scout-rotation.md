...
Next up by this rule: after the 2026-07-26 `efs` re-pass, the oldest built-adapter doc is
`truckercloud` (2026-07-20), but anonymous scouting there is exhausted (five straight total
403 walls) — skip it until a human makes vendor contact. The next-oldest group is a
three-way tie at 2026-07-22 (`mailbox`, `wex`, `comdata`); **prefer `comdata` next**
(alphabetical tiebreak, same rule style as prior ties in this doc). Worth flagging for
whoever reviews this rotation: `efs` itself just logged its third straight fully-walled
pass (07-20 found one new corroborating field via snippet; 07-24 and 07-26 found nothing
new at all) — the anonymous-scouting well for `efs` is close to the same exhaustion point
`truckercloud` hit at five, and the marginal value of another blind pass on any
already-3-plus-walled provider is dropping fast. A meta-governor pass over this rotation's
last few weeks of "no adapter-breaking change, every vendor page 403s" cycles may find it's
due for a slower cadence, not a fix.
