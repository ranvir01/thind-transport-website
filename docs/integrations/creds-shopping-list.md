...
| 6 | QuickBooks Online | QBO plan + free Intuit developer app | developer.intuit.com | Pulls QBO payments onto matching invoices (by number) through the same `recordPayment` path the office UI uses — status, audit, load-status cascade all included | **adapter shipped** — plumbing + contract tests done in `src/lib/hub/integrations/qbo.ts`, including automatic refresh-token rotation persistence (no migration needed — see `docs/integrations/qbo.md`); confirm the real sandbox response shape before flipping to live. Invoice push (LoadOff → QBO) not built yet |
