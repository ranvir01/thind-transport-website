...
| 7 | Factoring company API | Ask your factor (varies) | your factor | Electronic invoice submission + advance/reserve tracking | stub — the credential CHECK-constraint blocker is gone (migration 014 relaxed it to a shape check), same as QBO; still needs its own adapter slice (webhook receiver + push action) |
