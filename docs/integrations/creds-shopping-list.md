...
| 2 | Docs mailbox (IMAP) | Free (use an existing mailbox + app password) | Gmail app-password settings (Google Account → Security → App passwords) | Rate cons/PODs auto-file to the matching load by subject reference | **live for Gmail today; Office 365/Exchange Online will NOT work** — Microsoft disabled IMAP basic auth in Oct 2022 and the adapter (`src/lib/hub/mailbox.ts`) has no OAuth2 path yet. See `docs/integrations/mailbox.md`. |
