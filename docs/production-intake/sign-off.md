...
| `npm run e2e:sweep` | PASS | Against HTTPS tunnel: owner sign-in, `/hub`, `/hub/loads`, `/hub/money`, `/hub/fleet`, `/hub/ranker`, `/hub/reports`, report builder, `/hub/onboarding`, setup pages, driver PWA tabs/offline page, `/track/sandbox`, manifest, service worker, and CSV exports all returned success. |
