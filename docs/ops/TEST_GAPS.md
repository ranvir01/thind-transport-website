...
| 12 | `src/lib/hub/settlements.ts:71` `latestScorecardScore` → `src/lib/hub/recruiting.ts:395` `computeDriverScores` — **test coverage now added** (`draft-settlements-loads.test.ts`'s "adds a scorecard bonus at the matching tier" case, landed 2026-07-26), pinning tier selection (sort desc, first `>=`) through `draftSettlements` end to end. **Still open**: `computeDriverScores` itself has no dedicated test, and no `scorecard_bonus` rule exists in the 11 seeded `hub.pay_rules` rows | The `scorecard_bonus` chain: monthly cron (`vercel.json` `driver-scorecards`, `0 10 1 * *`) → `hub.driver_scores` → tier lookup — the tier-selection math is now regression-guarded, but the feature is unused in the seed/production rule set | Drivers (bonus never paid, or wrong tier), if the feature is ever turned on | Bonus amount is per-tier and carrier-set — sizing needs the intended tier table from Ranvir; not a code gap anymore, a product/seeding decision | `computeDriverScores` (`recruiting.ts:395`) itself; separately assert `latestScorecardScore` returns `null` when `to_regclass` is null (`:75`) | 15 |
