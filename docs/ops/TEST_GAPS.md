...
> owner decision), #14 (`parseRuleSet` branch coverage — a test block exists at `pay-rules.test.ts:295`
> but wasn't checked against all four cases the row below specifies; the underlying fix also needs an
> owner call on throw-vs-drop for malformed pay rules).
> **Update, verify-and-build cycle, 2026-07-26 ~22:40 UTC:** **#13 closed.** `sendFactoringPacket`
> (`invoices.ts:540`) and its caller `factoringPacketAction` (`money.ts:87`) were both 0% covered.
> Read the function before writing anything: it already does what the row asked for — a missing
> attachment throws (naming the file) instead of shipping an incomplete packet, a missing factoring
> email is refused up front, and `factoringPacketAction` catches the throw and returns `{ok:false,
> error}` rather than swallowing it. No bug found; added `send-factoring-packet.test.ts` (invoice-not-
> found, missing-email, happy-path attachment set + sent_log/audit stamp, missing-document throw —
> confirmed this last case actually fails when the `missing.length > 0` guard is removed, then restored)
> and `factoring-packet-action.test.ts` (carrierId/permission wiring, send-failure passthrough, Forbidden
> short-circuit). One open question for the owner, not fixed here: the packet attaches `rate_confirmation`
> + `pod` only, per the function's own doc comment — it does **not** attach the `bol` document kind even
> though `bol` exists as a document kind and some factors require it. Pinned the current (BOL-excluded)
> behavior rather than guessing; if factors need the BOL, that's a one-line change to the `.filter(...)`
> call at `invoices.ts:566` plus a test update, gated on an owner decision per AGENTS.md §5.
...
| 13 | ~~`src/lib/hub/invoices.ts:540` `sendFactoringPacket` — **0% covered**~~ **CLOSED 2026-07-26**: `send-factoring-packet.test.ts` and `factoring-packet-action.test.ts` cover invoice-not-found, missing-factoring-email, the happy-path attachment set (invoice PDF + rate confirmation + POD) with `sent_log`/audit stamping, and a missing-attachment throw (confirmed to actually catch the regression). No bug found — a send failure already surfaces as `{ok:false}`, not a swallowed catch. Open question for the owner, not a bug: the packet excludes the `bol` document kind by design (per the function's own doc comment); flagged in the note above rather than changed. | The packet that gets an invoice *funded*. If it silently fails, the invoice sits unfunded and nobody finds out | Ranvir (cash timing) | Was **$2,973** avg open invoice ($8,920 / 3) held ~7 extra days ≈ **$7** carrying cost *at 12% APR* — now regression-guarded | — | 25 |
