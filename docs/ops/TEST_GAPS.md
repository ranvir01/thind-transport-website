...
>
> **Still open:** #11 (detention downward-revision — owner decision), #12 (scorecard tier table —
> owner decision), #14 (`parseRuleSet` branch coverage — a test block exists
> at `pay-rules.test.ts:295` but wasn't checked against all four cases the row below specifies).
>
> **Resolved 2026-07-26 (this cycle):** #8 `getAgingSummary` — `aging-summary.test.ts` landed
> (`fd8d1c03`), pins the `status NOT IN ('paid')` + `carrier_id` scoping and the bucket/open_cents math.
> #13 `sendFactoringPacket` (`invoices.ts:540`) — `send-factoring-packet.test.ts` pins the happy-path
> attachment set (invoice PDF + rate confirmation + POD), confirms a missing document rejects with a
> named-file error instead of sending an incomplete packet (the code was already correct — no swallowed
> catch — now regression-guarded), and the no-factoring-email / invoice-not-found guards.
...
| 13 | ~~`src/lib/hub/invoices.ts:488` `sendFactoringPacket` — **0% covered** (lines 493–529)~~ **CLOSED 2026-07-26**: `send-factoring-packet.test.ts` asserts the packet attaches invoice PDF + rate confirmation + POD to `settings.factoring.email`, and that a missing document **rejects** (throws with the missing filename named) rather than sending an incomplete packet or swallowing the error. No bug found — the code already threw correctly; now regression-guarded. The caller `factoringPacketAction@87` remains 0% covered (permission-wiring gap, see #3's sibling actions). | Ranvir (cash timing) | Was **$2,973** avg open invoice ($8,920 / 3) held ~7 extra days ≈ **$7** carrying cost *at 12% APR* — now regression-guarded | — | 25 |
