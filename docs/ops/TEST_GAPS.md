...
| **`reports.ts`** | **56.6** | 57.1 | 45.5 | 3 | `arAgingTrend@53`, `settlementLiability@122`, `fuelSpendSummary@375`, `exportFuelSpendCsv@421` all 0% |
...
| 5 | `src/lib/hub/pay-rules-db.ts:21` `syncDefaultPayRules` + `:63` `getActivePayRules` — **0% covered, zero test file imports this module** | `:32` `if (custom) return` is the only thing stopping a routine driver-form save from **clobbering a hand-built pay program**. `:73` `ORDER BY (name = ANY($3)) ASC` is the only thing making custom beat auto. Both are one-token-fragile and unasserted | The driver whose custom program vanishes | **$236/week fleet-wide** for a 2-point percentage delta on $11,801 (≈**$25/week** for one driver at $1,243 avg gross, `hub.settlements`); unbounded if a bespoke program (per-stop, scorecard, flat) silently reverts | Two fake-`pg` tests: (a) a custom-named active row makes `syncDefaultPayRules` issue **zero** writes; (b) `getActivePayRules` returns the custom row when both exist. Assert the exact `ORDER BY` | 25 |
| 6 | `src/lib/hub/settlements.ts:267-272` — the advance-apply `UPDATE` loop is **0% covered** (`approveSettlement` itself is partially covered) | An advance deducted on the statement but not flipped to `applied` deducts **again** on the next settlement. The `AND status = 'outstanding'` guard at `:269` is the only idempotency | Drivers | **$1,500/driver** = the cap at `advances-core.ts:32`; ×10 drivers = **$15,000**. Seeded reality today is one $200 advance | Fake client: settlement with an `advance` line → assert the `UPDATE hub.advances` fired with `[settlementId, source_id, carrierId]` and the `status='outstanding'` guard present; re-run and assert 0 rows | 20 |
| 7 | `src/lib/hub/reports.ts:53` `arAgingTrend` and `:122` `settlementLiability` — both **0% covered** | The two numbers the owner reads before deciding whether to make payroll. `arAgingTrend`'s payments sub-select (`:76-79`) has **no `carrier_id` filter** — every other payments roll-up in the repo does (`invoices.ts:14`, `invoices.ts:371`) | Ranvir (a wrong cash decision) | Not a direct loss — a **decision** error on an $8,920 AR book and an $11,801 payroll liability. Rank it high because it is 20 min of work | Fixture-row test: two invoices, one partially paid, at known checkpoint dates → assert bucket totals and `totalOpenCents`. Add `AND p.carrier_id = i.carrier_id` while you are in there | 20 |
| 8 | `src/lib/hub/invoices.ts:287` `getAgingSummary` — **0% covered** | Feeds both the AR page and `runOverdueReminders` (#2). `openCents = amount_cents - (paid_cents ?? 0)` at `:297`; `paid_cents` comes from the sub-select at `:14`. A partially-paid invoice mis-bucketing means you chase money you already have, or don't chase money you don't | Ranvir + the customer relationship | Silent: **$3,560** (`THD-INV-1004`, the one seeded fully-paid invoice) chased after payment, or a real balance never chased. Largest *open* invoice is $3,440 | Three invoices — unpaid, half-paid, over-paid — assert `buckets`, `open_cents` and that fully-paid rows are excluded by the `status NOT IN ('paid')` filter at `:289` | 20 |
| 9 | `src/lib/hub/loads.ts:212` `createLoad` / `:266` `updateLoad` — **0% covered** (`loads.ts` is 35.2% stmt / **14.4% branch**) | Every load's `linehaul_cents`, `fuel_surcharge_cents` and `accessorials` JSONB is written here, and this is where `assertCarrierRefs` (`tenancy.ts:26`) is called. Nothing pins that the tenancy guard is still invoked on create/update | Ranvir (mis-billed load) or cross-tenant leak | **$2,447** per load (avg linehaul) × 27 loads if a units bug lands = **$66,067** blast radius | Fake client: `createLoad` with a customer id from another carrier → assert it throws `"Customer not found"` before any `INSERT`. Then a happy path asserting the cents columns are integers | 30 |
| 10 | `src/lib/hub/fuel.ts:96` `assignFuelToLoad` and `:196` `fuelFraudFlags` — **0% covered** (`fuel.ts` 12.1% stmt, 9.1% func) | `assignFuelToLoad` carries its tenancy guard *inside the SQL join* (`:110-114`) — no helper, no test. `fuelFraudFlags` is the only duplicate-swipe / over-tank-capacity detector in the product | Ranvir (card fraud goes unseen) | A duplicate fuel swipe on a 10-truck fleet: **$400–$600** per event (*assumption: one 100-gal fill ≈ $400 at $4/gal; frequency unknown — MISSING: pull 90 days of EFS/WEX/Comdata settlement files and count true duplicates*) | Fake client returning two rows 20 min apart with identical `total_cents` → assert one `duplicate` flag; a `gallons > tank_capacity_gallons` row → assert one `over_capacity` flag. Assert both queries carry `carrier_id = $1` | 25 |
...
| 14 | `src/lib/hub/pay-rules.ts:424` `parseRuleSet` — branches `:429,:430` uncovered; the doc-comment says "defensive parse" but the body is `row.rules as PayRule[]` | A malformed `rules` JSONB (a UI write, a manual `psql` edit, a future migration) passes straight into `evaluatePayRules`. `basisPoints: 90` instead of `9000` pays **0.9% instead of 90%** with no error | Drivers | **$2,138 per load** = $2,400 median linehaul × (90% − 0.9%). Silent — the statement just looks small | Feed `parseRuleSet` a rule with `basisPoints > 10000`, a negative rate, a string `basisPoints`, and an unknown `type` → assert it throws or drops the rule. Then assert `evaluatePayRules` never emits a line from an invalid rule | 30 |
