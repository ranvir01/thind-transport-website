...
>
> **Doc-hygiene pass, 2026-07-27 (verify-and-build cycle):** the §2 table below had drifted from this
> narrative and from itself — rows 5/6/7 and 8/9/10 were each duplicated (an open copy sitting right next
> to the closed one, from a merge that kept both sides), and #2/#3/#4/#13/#15 stayed marked open in the
> table long after the paragraphs above closed them. Re-verified every row against the current tree (test
> files exist and cover the described cases, or don't) rather than trusting either version: deleted the
> six duplicate rows, closed #2/#3/#4/#13 in the table to match this narrative, and corrected #15 — despite
> two later paragraphs above claiming `csvEscape` was "still duplicated three ways", the code has held one
> shared implementation in `csv.ts` since `a0406ae2`; those later claims were themselves stale, not this
> fix. #12 is split: `draftSettlements`' tier-selection math now has real test coverage (2026-07-26), but
> `computeDriverScores` itself is still untested and no `scorecard_bonus` rule is seeded — that half stays
> open as a product decision, not a code gap. **Confirmed still genuinely open:** #11 (detention downward
> revision — needs an owner-decided recompute/override design) and #12's `computeDriverScores` half. No
> other numbered row in §2 has an unresolved code-coverage gap as of this cycle.
>
> **Verify-and-build, 2026-07-27 (routine cycle):** closed the code-coverage half of #12.
> `computeDriverScores` (`recruiting.ts:451`) had zero dedicated tests — only mocked out in
> `cron-route.test.ts` — despite feeding the exact composite score `latestScorecardScore` reads
> for the scorecard-bonus tier lookup. Added `compute-driver-scores.test.ts`: pins the composite
> formula (70% on-time + 30% MPG-vs-fleet capped at 1.2x, minus 15pts/incident, clamped 0-100),
> the zero-delivery/zero-incident skip (`continue`, no row written), the zero-delivery-with-incidents
> path (`onTimePct` defaults to 100), the `mpgRatio` cap, the lower clamp, and the `{drivers: N}`
> return count including skipped drivers. Verified the formula assertion actually catches a
> regression (flipped `incidents * 15` to `* 10`, one test failed with a clear diff, then restored).
> **#12 remains partially open**: the `scorecard_bonus` tier-sizing table (0 of the 11 seeded
> `hub.pay_rules` rows use it) is still a product/seeding decision for Ranvir, not a code gap.
...
| 2 | ~~`src/lib/hub/invoices.ts:326` `if (![3, 10, 20].includes(daysPast)) continue` inside `runOverdueReminders@310` — **0% covered**~~ **CLOSED**: `nextReminderRung` (`invoices.ts:345`) replaced the exact-day check with a highest-crossed-rung-not-yet-climbed comparison; `overdue-reminder-ladder.test.ts` (`5c158d72`) exercises the rung-skip, double-send and 22-day-drift cases this row described. No bug found; now regression-guarded. | Dunning fires only on days 3, 10 and 20 *exactly*. Miss the cron once and that invoice is never chased again | Ranvir (cash) | Was **$2,930** worst case — now regression-guarded | — | 30 |
| 3 | ~~`src/app/hub/_actions/money.ts` — 12 of 14 actions 0% covered; the `requirePermission` string is never asserted~~ **CLOSED**: `money-actions-permissions.test.ts` (`53f2ca8a`) table-drives all 14 actions including the approve-vs-write tier split and asserts the underlying `settlements`/`invoices` function is never called when `requirePermission` throws. No bug found; now regression-guarded. | Change an approve-tier action to `money:read` and a dispatcher could approve settlements — every other test would still pass | Ranvir (fraud/error surface) | Was **$11,801** exposure — now regression-guarded | — | 40 |
| 4 | ~~`src/lib/hub/expenses.ts:199` `const year = new Date().getFullYear()` in `exportCsv` case `"1099"` — **0% covered**~~ **CLOSED**: `resolve1099Year` (`expenses.ts:117`) replaced the raw current-year read with a default-to-prior-year resolver plus a `?year=` passthrough; `export-1099-year.test.ts` (`5c158d72`) pins both. No bug found; now regression-guarded. | The 1099-NEC export always used the **current** year — in Jan 2027 the button would have returned an empty CSV | Ranvir + accountant | Was **$240–$1,360** in IRS §6721 penalty exposure — now regression-guarded | — | 25 |
...
| 12 | `src/lib/hub/settlements.ts:71` `latestScorecardScore` → `src/lib/hub/recruiting.ts:451` `computeDriverScores` — **CLOSED (code coverage), 2026-07-27**: `compute-driver-scores.test.ts` pins the composite formula, the zero-delivery/zero-incident skip, the zero-delivery-with-incidents path, the `mpgRatio` cap, the lower clamp, and the driver count return. `latestScorecardScore`'s tier selection was already pinned end to end via `draftSettlements` (2026-07-26). **Still open, not a code gap**: no `scorecard_bonus` rule exists in the 11 seeded `hub.pay_rules` rows | The `scorecard_bonus` chain: monthly cron (`vercel.json` `driver-scorecards`, `0 10 1 * *`) → `hub.driver_scores` → tier lookup — both the scoring math and the tier-selection math are now regression-guarded, but the feature is unused in the seed/production rule set | Drivers (bonus never paid, or wrong tier), if the feature is ever turned on | Bonus amount is per-tier and carrier-set — sizing needs the intended tier table from Ranvir; not a code gap, a product/seeding decision | — | 15 |
| 13 | ~~`src/lib/hub/invoices.ts:488` `sendFactoringPacket` — **0% covered**~~ **CLOSED**: `send-factoring-packet.test.ts` (`17015b0f`) asserts the packet includes invoice PDF + POD + BOL, cross-carrier invoice ids throw, and a send failure surfaces as a thrown error rather than a swallowed catch; `factoring-packet-action.test.ts` (`f6255af2`) covers the action-layer permission gate. No bug found; now regression-guarded. | The packet that gets an invoice *funded*. If it silently failed, the invoice would sit unfunded with nobody finding out | Ranvir (cash timing) | Was **$2,973** avg-invoice cash-timing exposure — now regression-guarded | — | 25 |
...
| 15 | ~~`src/lib/hub/expenses.ts:102` / `src/lib/hub/reports.ts:407` / `src/lib/hub/loadboard-export.ts:4` — three duplicate `csvEscape` implementations, none escape a leading `=`/`+`/`-`/`@`~~ **CLOSED**: `csv.ts` (`a0406ae2`) holds one shared `csvEscape` guarding a leading `=`/`+`/`-`/`@`; confirmed this cycle (2026-07-27) that all three former call sites (`expenses.ts`, `reports.ts`, `loadboard-export.ts`) import it from `./csv` and no local re-implementation remains. `csv.test.ts` covers the escape. No bug found; now regression-guarded. This row had been marked "still open"/"still duplicated" in two later status updates above without re-checking the code — those were stale, not this fix. | A customer name typed as `=HYPERLINK(...)` becoming a live formula when the bookkeeper opens `invoices.csv` in Excel | Whoever opens the CSV | Was **≈$0 direct** exposure, labeled small on purpose — now regression-guarded | — | 15 |
