...
| 7 | **Smoke the doc-intake LLM once with a real `ANTHROPIC_API_KEY`.** Nothing in this repo can prove `claude-sonnet-5` accepts this request shape — every test mocks `fetch`. The new `console.error` on a non-ok response is what will surface a failure; watch the logs on the first real document. **Widened 2026-08-30:** the Inbox (emailed rate cons, ADR 0005) now runs the same `analyzeDocumentEnhanced` path, so this key improves every parse the mailbox stages, not just Smart Setup uploads. Unset is safe — heuristics, silently. |
| 7b | **Set a fresh Gmail App Password in `SMTP_USER`/`SMTP_PASS`.** Failing with `535-5.7.8 BadCredentials` since 2026-08-08 across 5 carriers (3 non-seed), which breaks compliance alerts. It also gates the Inbox: no mailbox credential means no inbound rate cons to stage, so `/hub/inbox` stays empty for real freight. |
