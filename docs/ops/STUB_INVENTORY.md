...
**Staleness pass 2026-07-27:** re-read every citation in §4's defects table plus the qbo/dat
status labels against the current working tree. **8 of the 11 defects below were already fixed**
by commits that landed after this doc's 2026-07-25 audit (some by minutes) — marked inline as
**FIXED** with the commit and current line numbers; struck-through text is the original finding,
kept for context. Two (Terminal webhooks, QBO's missing `authorization_code` grant) are confirmed
still open. One (Truckstop) is half-fixed — the element-name guess is now tolerant of both names,
but the sandbox-default base URL is unchanged. qbo/dat registry labels are corrected (`registry.ts:138`,
`:87` — qbo is now `status: "live"`, dat is now `status: "stub"`). Section 0's headline
...
|---|---|---|
| **High — FIXED** | ~~`hasCredentials()` does not check `credentialsConfigured()`.~~ `getCredentials()` returns `null` when `CREDENTIALS_KEY` is missing, but `hasCredentials()` used to query the table directly, so a rotated/dropped key left every adapter's `connected()` reporting **true** while `pull()` threw forever, silently. `hasCredentials()` (`credentials.ts:75-83`) now carries the same `credentialsConfigured()` guard, with a comment naming this exact failure mode. | `credentials.ts:75-83` |
| **High — FIXED 2026-07-25** | ~~IFTA jurisdiction is computed from a two-character truncation.~~ `normalizeState()` was `value.trim().toUpperCase().slice(0, 2)`, so a full state name mis-mapped (Minnesota→MI, Alaska→AL, Nevada→NE, Montana→MO) and `computeIftaQuarter` credited the wrong jurisdiction. Fixed by `5c158d72` (landed ~1 minute after this doc's audit pass) with a name→code table (`csv.ts:338-350`, `JURISDICTION_BY_NAME`) plus a named regression test for exactly these six collisions (`__tests__/csv.test.ts:94-107`). | `csv.ts:338-350` |
| **Medium — FIXED** | ~~EFS drops fuel rows silently.~~ `ingestWexRows`/`ingestComdataRows` guarded `if (!row.external_id) continue`; `ingestEfsRows` didn't, so a feed row missing `TransactionId` and every later empty-id row collided on the unique index and vanished into `skipped` with no explanation. `efs.ts:112-116` now carries the identical guard, with a comment naming WEX/Comdata as precedent. | `efs.ts:112-116` |
| **Medium — FIXED** | ~~EFS is the only fuel adapter that doesn't normalize state at all.~~ `wex.ts`/`comdata.ts` called `normalizeState`; `efs.ts` passed `record.MerchantState` through raw, so "Wash." and "WA" filed as different IFTA jurisdictions on the card actually in use (35 of 36 rows). `efs.ts:57` now calls `normalizeState(String(record.MerchantState))` too. | `efs.ts:57` |
| **Medium — FIXED 2026-07-27** | ~~The docs mailbox files every attachment as a rate confirmation.~~ `saveDocument` was called with a hardcoded `kind: "rate_confirmation"` for every attachment on every matched message — no sender, filename, or subject inference. The registry blurb promises "files rate cons/**PODs**" (`registry.ts:66`). Fixed by `classifyDocumentKind()` (`mailbox.ts:45-65`, tested in `__tests__/mailbox-classify.test.ts`), landed in `78281b53` before this doc's audit pass completed. | `mailbox.ts:45-65`, `:124` |
...
| **Low — HALF FIXED** | **Truckstop search fails open, against the sandbox.** `parseLoadSearchResponse` throws on a SOAP `<faultstring>` but used to return `[]` for the one wrong element-name guess. `truckstop.ts` now tries **both** `LoadSearchResult` and `LoadSearchItem` (`LOAD_ITEM_TAGS`, fixed in `eed32214`, 2026-07-26), so a real response can no longer silently parse to zero postings over a name guess. Still open: the default base remains `https://testws.truckstop.com` (`truckstop.ts:247`) — the sandbox — so first contact still renders "no loads found" against test data rather than an error a dispatcher can report. | `truckstop.ts:247` (open); `LOAD_ITEM_TAGS` (fixed) |
| **Low — not reproducible against current code** | **Three adapters reimplement `dollarsToCents` inline.** Re-checked `comdata.ts`, `dat.ts`, `truckstop.ts`: all three call the shared `roundHalfAwayFromZero` helper today, not a bare `Math.round(x * 100)`, and all three files' git history shows them created already using it (2026-07-22, three days before this doc). Money-conversion is consistent across the three adapters as of this pass. | `roundHalfAwayFromZero` call sites in each adapter |
| **Low — FIXED** | ~~One float leaves the building in a money path.~~ `factor.ts` used to send `amount: invoice.amount_cents / 100` in the submission JSON — binary division is the wrong direction of travel for a money value even though nothing is stored as a float. `factor.ts:142` now sends `centsToDecimalString(invoice.amount_cents)`, with a comment naming the exact rounding-artifact risk this closes. | `factor.ts:142` |
