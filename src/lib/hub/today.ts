    ...
    await Promise.all([
      ...
      query<{ id: string; unit_number: string; driver_name: string | null; where_city: string | null; where_state: string | null }>(
        `SELECT t.id, t.unit_number, d.first_name || ' ' || d.last_name AS driver_name,
         ...
         LEFT JOIN LATERAL (
           SELECT s.city, s.state FROM hub.stops s JOIN hub.loads l ON l.id = s.load_id AND l.carrier_id = s.carrier_id
           WHERE l.truck_id = t.id AND l.carrier_id = t.carrier_id AND s.type = 'delivery' AND s.departed_at IS NOT NULL
           ORDER BY s.departed_at DESC LIMIT 1
         ) lp ON TRUE
         WHERE t.carrier_id = $1 AND t.deleted_at IS NULL AND t.status = 'active'
           AND NOT EXISTS (
             SELECT 1 FROM hub.loads l WHERE l.truck_id = t.id AND l.carrier_id = t.carrier_id AND l.deleted_at IS NULL
               AND l.status IN ('booked','dispatched','at_pickup','in_transit')
           )
