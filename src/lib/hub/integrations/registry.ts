...
export const PROVIDERS: readonly ProviderSpec[] = [
  ...
  },
  {
    ...
    blurb: "Polls an inbox and files rate cons/PODs onto matching loads by reference number in the subject. Gmail: paste an app password below. Microsoft 365 / Google Workspace: leave App password blank and fill in the OAuth2 fields instead (Exchange Online and Workspace both rejected plain-password IMAP login in 2022/2025) — see docs/integrations/mailbox.md.",
    fields: [
      { key: "host", label: "IMAP host (e.g. imap.gmail.com)" },
      { key: "port", label: "Port (993)" },
      { key: "user", label: "Mailbox user" },
      { key: "password", label: "App password (Gmail — leave blank if using OAuth2 below)", secret: true },
      { key: "folder", label: "Folder (default INBOX)" },
      { key: "tokenEndpoint", label: "OAuth2 token endpoint (Microsoft 365 / Workspace only)" },
      { key: "clientId", label: "OAuth2 client ID (Microsoft 365 / Workspace only)" },
      { key: "clientSecret", label: "OAuth2 client secret (Microsoft 365 / Workspace only)", secret: true },
      { key: "refreshToken", label: "OAuth2 refresh token (Microsoft 365 / Workspace only)", secret: true },
    ],
