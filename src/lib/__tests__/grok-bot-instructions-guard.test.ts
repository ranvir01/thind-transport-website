...
describe("grok-bot instruction files (four live bots, paste-ready, ≤4k)", () => {
  it("ships SETUP.md as the one owner file plus the four instruction bodies", () => {
    ...
    expect(setup).toMatch(/D-012/)
    expect(setup).toMatch(/Chief of Staff/)
    expect(setup).toMatch(/Staff SRE/)
    expect(setup).toMatch(/Head of RevOps/)
    expect(setup).toMatch(/Expense Manager analog/)
    expect(setup).toMatch(/Talent Scout/)
  ...
  it("Rav is proof-only: no outreach, no scraping, claims map to open links", () => {
    ...
    expect(rav).toMatch(/Chief of Staff/)
