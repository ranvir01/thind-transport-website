...
const HARD_GOTO_COPY: Record<string, string> = {
  ...
  // Nested /hub/driver/incident before /hub/driver so the PWA home still
  // maps to its own "Last pay" glance. Title "Report an incident" is the
  // More-menu link label, not a DriverNav item (Home / Messages / Pay /
  // More), so waitForText on it is a real render gate today — still use
  // the unique subtitle fragment, same machinery as list pages. Always-on
  // (the form has no empty-state branch); "File the report" is a client-
  // component CTA that hydrates after the server subtitle, so it is not
  // the family gate. Forced-dark surface: never text-fg*/bg-surface*/
  // border-border*. src/app/hub/driver has no loading.tsx.
  "/hub/driver/incident": "starts the paper trail",
  // Nested /hub/driver/dvir before /hub/driver so the PWA home still
  ...
  // waitForText on it is a real render gate today — still use the unique
  // subtitle fragment, same machinery as list pages. Always-on (empty
  // list still shows it). Sweep already anchors on "book you over it".
  // "Ask for home time" lives on /hub/driver/more as the Time-off hint —
  // never use it as the family gate. "Ask for these days" / "#to-start"
  // are client-component form chrome that hydrates after the server
  ...
  // gate. There is no PageHeader subtitle; the Time-off hint "Ask for
  // home time" is unique destination copy on this page (the timeoff
  // subtitle starts with the same phrase on a different route). Always-on
  // (the link list has no empty-state branch). Sweep already anchors on
  // "ask for home time". Do NOT use the h1 "More", the other link hints
  // ("Pre/post-trip", "CDL, med card"), or SignOutButton copy. Forced-
  // dark surface: never text-fg*/bg-surface*/border-border*.
  // src/app/hub/driver has no loading.tsx.
  "/hub/driver/more": "ask for home time",
  // Nested /hub/driver/docs before /hub/driver so the PWA home still
  // maps to its own "Last pay" glance. Title "My documents" is the
  // More-menu link, not a DriverNav item (Home / Messages / Pay /
  // More), so waitForText on it is a real render gate today — still
  // use the unique subtitle, same machinery as list pages. Always-on
  // (empty list still shows it). Sweep currently anchors on "my
  // documents" (the title); the family gate is the subtitle. Do NOT
  // use "CDL, med card" (the More-page hint) or "No files yet" (empty
  // list only). Forced-dark surface: never text-fg*/bg-surface*/
  // border-border*. src/app/hub/driver has no loading.tsx.
  "/hub/driver/docs": "What the office has on file for you — and when it runs out.",
...
describe("e2e soft-nav landing gates wait for render before reading", () => {
  ...
  it("id-segment hard-goto keys match template interpolations, not named children", () => {
    ...
    expect(HARD_GOTO_PATH_RE.exec(line("/hub/driver/incident"))?.[1]).toBe("/hub/driver/incident")
    expect(HARD_GOTO_PATH_RE.exec(line("/hub/driver/dvir"))?.[1]).toBe("/hub/driver/dvir")
    expect(HARD_GOTO_PATH_RE.exec(line("/hub/driver/timeoff"))?.[1]).toBe("/hub/driver/timeoff")
    expect(HARD_GOTO_PATH_RE.exec(line("/hub/driver/more"))?.[1]).toBe("/hub/driver/more")
    expect(HARD_GOTO_PATH_RE.exec(line("/hub/driver/docs"))?.[1]).toBe("/hub/driver/docs")
  ...
  it("hard-goto list pages wait for destination copy, not the nav label", () => {
    ...
    expect(gates, "guard is not silently vacuous").toBeGreaterThanOrEqual(185)
    expect(misses).toEqual([])
  })
