...
const HARD_GOTO_COPY: Record<string, string> = {
  ...
  // link label (and the home glance link), not a DriverNav item (Home /
  // Messages / Pay / More), so waitForText on it is a real render gate
  // today — still use the unique subtitle fragment, same machinery as
  // list pages. Always-on (empty "Nothing yet" and loaded lists both
  // show the lead-in); sweep already anchors on "book you over it".
  // "Ask for home time" is the More-menu hint, not the family gate.
  // "Ask for these days" is a client-component CTA that hydrates after
  // the server subtitle, so it is not the family gate. Forced-dark
  // surface: never text-fg*/bg-surface*/border-border*.
...
describe("e2e soft-nav landing gates wait for render before reading", () => {
  ...
  it("id-segment hard-goto keys match template interpolations, not named children", () => {
    ...
    // /hub/driver/dvir, and /hub/driver/timeoff (extra slash). Office
    // roster must keep its own key; nested PWA children are listed
    // first so they map to their own subtitles, not the home glance.
    // Remaining nested PWA routes (more/docs) stay uncovered until
    // their own cycle. Thread detail stays uncovered — lookahead
  ...
  it("hard-goto list pages wait for destination copy, not the nav label", () => {
    ...
    expect(gates, "guard is not silently vacuous").toBeGreaterThanOrEqual(183)
    expect(misses).toEqual([])
  })
