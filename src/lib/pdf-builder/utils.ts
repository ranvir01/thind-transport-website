...
export const HIGHLIGHT_BG = rgb(1, 0.95, 0.85) // Light peach for highlighted fields
...
export function drawTextFieldVertical(
  ...
  field.addToPage(page, {
    ...
    width: width,
    height: height,
    borderWidth: required || highlight ? 2 : 1,
    backgroundColor,
  })
  field.setFontSize(FONT_LABEL)
...
export function drawInstructions(
  ...
  return drawWrappedText(page, ctx, text, x, y, width, {
    size: fontSize,
    color: GRAY,
  })
}

function wrapTextLines(ctx: PDFContext, text: string, width: number, fontSize: number): string[] {
...
/**
 * Draw wrapped text with custom styling
 */
export function drawWrappedText(
  ...
  const fontSize = options?.size || FONT_LABEL
  const lines = wrapTextLines(ctx, text, width, fontSize)
  let currentY = y
  const lineHeight = fontSize + WRAP_LINE_GAP
...
/**
 * Draw a highlighted callout box with wrapped text
 */
...
/**
 * Draw a wrapped Yes/No question with checkboxes aligned to the right
 */
export function drawYesNoQuestion(
  page: PDFPage,
  ctx: PDFContext,
  fieldBaseName: string,
  question: string,
  ...
  options?: {
    fontSize?: number
    yesNoColumnWidth?: number
  }
): number {
  const fontSize = options?.fontSize || FONT_LABEL
  const yesNoColumnWidth = options?.yesNoColumnWidth || 140
  const questionWidth = width - yesNoColumnWidth
  const checkboxX = x + questionWidth + 10
  const checkboxY = y - 2

  drawCheckbox(page, ctx, `${fieldBaseName}_yes`, "Yes", checkboxX, checkboxY)
  drawCheckbox(page, ctx, `${fieldBaseName}_no`, "No", checkboxX + 50, checkboxY)

  const nextY = drawWrappedText(page, ctx, question, x, y, questionWidth, {
    size: fontSize,
    color: BLACK,
  })
