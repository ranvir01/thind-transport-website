...
export function buildApplicantInfoPage(ctx: PDFContext): PDFPage[] {
  ...
  y -= 20
  drawText(page, ctx, "Required fields are highlighted in orange.", MARGIN_LEFT, y, { size: FONT_SMALL, color: GRAY })
  
  // Position applied for
  y -= 30
  y = drawSectionHeader(page, ctx, "POSITION APPLIED FOR", y)

  // Highlight that position selection is required
  y -= 5
  y = drawCallout(
    page,
    ctx,
    "IMPORTANT: Please check the position(s) you are applying for:",
    MARGIN_LEFT,
    y,
    CONTENT_WIDTH,
    FONT_LABEL
  ...
  y -= 5
  drawTextFieldVertical(page, ctx, "applicant_name", "Full Legal Name (Last, First, Middle)", MARGIN_LEFT, y - 20, 280, FIELD_HEIGHT, true, true)
  drawTextFieldVertical(page, ctx, "phone", "Phone Number", 350, y - 20, 120, FIELD_HEIGHT, true, true)
  drawTextFieldVertical(page, ctx, "emergency_phone", "Emergency Phone", 480, y - 20, 80, FIELD_HEIGHT)
  
  // Row 2: DOB, Age, SSN
  y -= 55
  drawTextFieldVertical(page, ctx, "dob", "Date of Birth", MARGIN_LEFT, y - 20, 100, FIELD_HEIGHT, true, true)
  drawTextFieldVertical(page, ctx, "age", "Age", 160, y - 20, 50, FIELD_HEIGHT)
  drawTextFieldVertical(page, ctx, "ssn", "Social Security Number", 220, y - 20, 130, FIELD_HEIGHT, true, true)
  drawTextFieldVertical(page, ctx, "email", "Email Address", 360, y - 20, 200, FIELD_HEIGHT, true, true)
  
  // Row 3: Physical Exam
  y -= 55
  drawTextFieldVertical(page, ctx, "physical_exam_exp", "DOT Physical Exam Expiration Date", MARGIN_LEFT, y - 20, 180, FIELD_HEIGHT, true, true)
  drawTextFieldVertical(page, ctx, "med_card_state", "Medical Card State", 250, y - 20, 100, FIELD_HEIGHT, false, true)
  ...
  // Address rows (3) - highlight current address (row 1)
  for (let i = 1; i <= 3; i++) {
    y -= 22
    ...
    addrField.addToPage(page, { x: MARGIN_LEFT, y: y - 2, width: 295, height: 18, backgroundColor: bgColor, borderWidth: borderWidth })
    addrField.setFontSize(9)

    const fromField = ctx.form.createTextField(`addr${i}_from`)
    fromField.addToPage(page, { x: MARGIN_LEFT + 300, y: y - 2, width: 75, height: 18, backgroundColor: bgColor, borderWidth: borderWidth })
    fromField.setFontSize(9)

    const toField = ctx.form.createTextField(`addr${i}_to`)
    toField.addToPage(page, { x: MARGIN_LEFT + 380, y: y - 2, width: 75, height: 18, backgroundColor: bgColor, borderWidth: borderWidth })
  ...
  drawText(page, ctx, "Post Grad:", MARGIN_LEFT + 365, y)
  for (let i = 1; i <= 4; i++) {
    drawCheckbox(page, ctx, `edu_postgrad_${i}`, `${i}`, MARGIN_LEFT + 420 + (i - 1) * 22, y - 2)
  }
  ...
  // Highlight that legal status is required
  y -= 5
  y = drawCallout(
    page,
    ctx,
    "REQUIRED: Answer all legal status questions below:",
    MARGIN_LEFT,
    y,
    CONTENT_WIDTH,
    FONT_LABEL
