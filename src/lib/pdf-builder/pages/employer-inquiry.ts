...
function buildEmployerInquiryForm(ctx: PDFContext, employerNum: number): PDFPage {
  ...
  drawText(page, ctx, "DOB:", 455, y, { size: FONT_SMALL, color: GRAY })
  const dobField = ctx.form.createTextField(`inq${employerNum}_dob`)
  dobField.addToPage(page, { x: 485, y: y - 4, width: 70, height: 16, backgroundColor: FIELD_BG })
  ...
  drawText(page, ctx, "Reason for Leaving:", 345, y)
  const reasonField = ctx.form.createTextField(`inq${employerNum}_reason`)
  reasonField.addToPage(page, { x: 455, y: y - 4, width: 100, height: 16, backgroundColor: FIELD_BG })
  ...
  drawText(page, ctx, "Title:", 325, y)
  const titleField = ctx.form.createTextField(`inq${employerNum}_resp_title`)
  titleField.addToPage(page, { x: 355, y: y - 4, width: 155, height: 16, backgroundColor: FIELD_BG })
