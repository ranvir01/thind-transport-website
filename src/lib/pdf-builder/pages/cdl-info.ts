...
export function buildCDLInfoPage(ctx: PDFContext): PDFPage[] {
  ...
  // License rows (3 entries) - highlight first row (current license)
  for (let i = 1; i <= 3; i++) {
    y -= 28
    x = MARGIN_LEFT
    const isCurrentLicense = i === 1
    const bgColor = isCurrentLicense ? HIGHLIGHT_BG : FIELD_BG
    const borderWidth = isCurrentLicense ? 2 : 1

    const stateField = ctx.form.createTextField(`lic${i}_state`)
    stateField.addToPage(page, { x: x, y: y - 2, width: 53, height: 22, backgroundColor: bgColor, borderWidth: borderWidth })
    stateField.setFontSize(10)
    x += 55

    const numField = ctx.form.createTextField(`lic${i}_number`)
    numField.addToPage(page, { x: x, y: y - 2, width: 128, height: 22, backgroundColor: bgColor, borderWidth: borderWidth })
    numField.setFontSize(10)
    x += 130

    const classField = ctx.form.createTextField(`lic${i}_class`)
    classField.addToPage(page, { x: x, y: y - 2, width: 53, height: 22, backgroundColor: bgColor, borderWidth: borderWidth })
    classField.setFontSize(10)
    x += 55

    const endorField = ctx.form.createTextField(`lic${i}_endorsements`)
    endorField.addToPage(page, { x: x, y: y - 2, width: 98, height: 22, backgroundColor: bgColor, borderWidth: borderWidth })
    endorField.setFontSize(10)
    x += 100

    const restrictField = ctx.form.createTextField(`lic${i}_restrictions`)
    restrictField.addToPage(page, { x: x, y: y - 2, width: 88, height: 22, backgroundColor: bgColor, borderWidth: borderWidth })
    restrictField.setFontSize(10)
    x += 90

    const expField = ctx.form.createTextField(`lic${i}_exp`)
    expField.addToPage(page, { x: x, y: y - 2, width: 78, height: 22, backgroundColor: bgColor, borderWidth: borderWidth })
  ...
  y = drawCallout(
    page,
    ctx,
    "IMPORTANT: Answer YES or NO to each question below. If YES, explain in detail.",
    MARGIN_LEFT,
    y,
    CONTENT_WIDTH,
    FONT_SMALL
