...
export function buildAccidentsViolationsPage(ctx: PDFContext): PDFPage[] {
  ...
  y = drawCallout(
    page,
    ctx,
    "REQUIRED: Fill in accidents below or check the NO accidents box.",
    MARGIN_LEFT,
    y,
    CONTENT_WIDTH,
    FONT_SMALL
  ...
  const accCols = [
    { text: "Date", width: 70 },
    { text: "Nature of Accident", width: 170 },
    { text: "Location", width: 120 },
    { text: "Fatal", width: 40 },
    { text: "Injuries", width: 45 },
    { text: "Hazmat", width: 40 },
  ]
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    dateField.addToPage(page, { x: x, y: y - 2, width: 68, height: 20, backgroundColor: FIELD_BG })
    dateField.setFontSize(10)
    x += 70

    const natureField = ctx.form.createTextField(`acc${i}_nature`)
    natureField.addToPage(page, { x: x, y: y - 2, width: 168, height: 20, backgroundColor: FIELD_BG })
    natureField.setFontSize(10)
    x += 170

    const locField = ctx.form.createTextField(`acc${i}_location`)
    locField.addToPage(page, { x: x, y: y - 2, width: 118, height: 20, backgroundColor: FIELD_BG })
    locField.setFontSize(10)
    x += 120

    const fatalField = ctx.form.createTextField(`acc${i}_fatalities`)
    fatalField.addToPage(page, { x: x, y: y - 2, width: 38, height: 20, backgroundColor: FIELD_BG })
    fatalField.setFontSize(10)
    x += 40

    const injField = ctx.form.createTextField(`acc${i}_injuries`)
    injField.addToPage(page, { x: x, y: y - 2, width: 43, height: 20, backgroundColor: FIELD_BG })
    injField.setFontSize(10)
    x += 45

    const hazField = ctx.form.createTextField(`acc${i}_hazmat`)
    hazField.addToPage(page, { x: x, y: y - 2, width: 38, height: 20, backgroundColor: FIELD_BG })
  ...
  y = drawCallout(
    page,
    ctx,
    "REQUIRED: Fill in violations below or check the NO traffic convictions box.",
    MARGIN_LEFT,
    y,
    CONTENT_WIDTH,
    FONT_SMALL
  ...
  const traffCols = [
    { text: "Date", width: 70 },
    { text: "Violation", width: 165 },
    { text: "Location", width: 115 },
    { text: "Vehicle", width: 70 },
    { text: "Penalty", width: 75 },
  ]
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    violField.addToPage(page, { x: x, y: y - 2, width: 163, height: 20, backgroundColor: FIELD_BG })
    violField.setFontSize(10)
    x += 165

    const locField = ctx.form.createTextField(`traffic${i}_location`)
    locField.addToPage(page, { x: x, y: y - 2, width: 113, height: 20, backgroundColor: FIELD_BG })
    locField.setFontSize(10)
    x += 115

    const typeField = ctx.form.createTextField(`traffic${i}_vehicle_type`)
    typeField.addToPage(page, { x: x, y: y - 2, width: 68, height: 20, backgroundColor: FIELD_BG })
    typeField.setFontSize(10)
    x += 70

    const penaltyField = ctx.form.createTextField(`traffic${i}_penalty`)
    penaltyField.addToPage(page, { x: x, y: y - 2, width: 73, height: 20, backgroundColor: FIELD_BG })
