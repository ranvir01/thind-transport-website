...
function buildMedicalInfoPage(ctx: PDFContext): PDFPage {
  ...
  drawText(page, ctx, "DOB:", 410, y)
  const dobField = ctx.form.createTextField("med_dob")
  dobField.addToPage(page, { x: 440, y: y - 4, width: 115, height: 18, backgroundColor: FIELD_BG })
...
function buildMedicalSelfCertPage(ctx: PDFContext): PDFPage {
  ...
  y = drawCallout(
    page,
    ctx,
    "IMPORTANT: Answer every question below and select Yes or No.",
    MARGIN_LEFT,
    y,
    CONTENT_WIDTH,
    FONT_SMALL
