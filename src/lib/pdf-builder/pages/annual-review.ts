...
function buildAnnualReviewForm(ctx: PDFContext): PDFPage {
  ...
  drawText(page, ctx, "Review Date:", 400, y)
  const reviewDateField = ctx.form.createTextField("annual_review_date")
  reviewDateField.addToPage(page, { x: 480, y: y - 4, width: 75, height: 18, backgroundColor: FIELD_BG })
  reviewDateField.setFontSize(10)
...
function buildDriverCertificationForm(ctx: PDFContext): PDFPage {
  ...
  dateField.addToPage(page, { x: 435, y: y - 4, width: 120, height: 18, backgroundColor: FIELD_BG })
  ...
  drawText(page, ctx, "Period:", 330, y, { size: FONT_SMALL, color: GRAY })
  const periodFromField = ctx.form.createTextField("cert_period_from")
  periodFromField.addToPage(page, { x: 365, y: y - 4, width: 60, height: 16, backgroundColor: FIELD_BG })
  periodFromField.setFontSize(9)
  
  drawText(page, ctx, "-", 430, y, { size: FONT_SMALL })
  const periodToField = ctx.form.createTextField("cert_period_to")
  periodToField.addToPage(page, { x: 440, y: y - 4, width: 60, height: 16, backgroundColor: FIELD_BG })
