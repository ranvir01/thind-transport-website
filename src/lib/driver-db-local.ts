 ...
 * local server, `driver-db-postgres.ts` swaps its `sql` tag for this one,
 * backed by the same node-postgres Pool the hub uses.
 */
import { Pool } from "pg"
...
export async function localSql(
  ...
  return localPool().query(q.text, q.values)
}
