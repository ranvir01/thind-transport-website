 ...
 * never let a failed write break the submission — email + DB are the source of truth.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    let savedToDb = false
    try {
      const parsedData: unknown = applicationData ?? { source: "driver-portal" }
      const saved = await saveApplication(session.user.id, parsedData)
      applicationId = saved.id
      savedToDb = true
      if (archivedPath) {
        await updateApplicationPDFPath(saved.id, archivedPath)
      }
    } catch (dbError) {
