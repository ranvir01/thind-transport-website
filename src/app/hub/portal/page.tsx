...
export default async function PortalHomePage() {
  ...
  return (
    <div className="space-y-5">
            ...
            {inTransit.map((load) => (
              <li key={load.id}>
                ...
                >
                  ...
                  <p className="mt-0.5 text-body-sm text-steel-100">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {invoice.pdf_url ? (
                      <a href={invoice.pdf_url} target="_blank" rel="noreferrer" aria-label="Download invoice"
                        className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/15 text-steel-100 hover:bg-white/5">
      ...
      {recent.length > 0 ? (
        <section>
          <h2 className="mb-2 font-display text-sm font-bold uppercase tracking-wider text-steel-300">
            Recent shipments
          </h2>
          <ul className="divide-y divide-white/5 rounded-2xl border border-white/10 bg-navy-800/80">
              ...
              <li key={load.id}>
                <Link href={`/hub/portal/loads/${load.id}`} className="flex items-center justify-between gap-2 p-3.5 hover:bg-white/5">
                  <span className="min-w-0 text-sm">
                    ...
                    <span className="text-steel-300"> · {load.origin_city} → {load.dest_city}</span>
                  </span>
          ...
          <div className="flex flex-wrap gap-2">
            {packet.map((doc) => (
              <a
                ...
                className="inline-flex min-h-[40px] items-center gap-1.5 rounded-xl border border-white/15 px-3 text-body-xs font-semibold text-steel-100 hover:bg-white/5"
              >
