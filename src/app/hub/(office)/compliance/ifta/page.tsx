    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <form method="GET">
          ...
          <button type="submit" className="ml-2 min-h-[44px] rounded-xl border border-white/15 px-4 text-sm font-semibold text-steel-100 hover:bg-white/5">
            Go
          </button>
        </form>
        ...
        {report ? (
          <div className="flex gap-2">
            ...
            <a href={`/api/hub/ifta/${quarter}/worksheet.csv`} className="inline-flex min-h-[44px] items-center gap-1.5 rounded-xl border border-white/15 px-4 text-sm font-semibold text-steel-100 hover:bg-white/5">
              CSV
            </a>
            <a href={`/api/hub/ifta/${quarter}/sources.csv`} className="inline-flex min-h-[44px] items-center gap-1.5 rounded-xl border border-white/15 px-4 text-sm font-semibold text-steel-100 hover:bg-white/5">
              4-yr source data
            </a>
          </div>
        ) : null}
      ...
      {report ? (
        <>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
            <Panel className="p-4">
              <span className="text-label text-steel-300 uppercase">Status</span>
              <p className="mt-1 font-display text-lg font-extrabold text-white uppercase">{report.status}</p>
            </Panel>
            <Panel className="p-4">
              <span className="text-label text-steel-300 uppercase">Mileage source</span>
              <p className="mt-1 font-display text-lg font-extrabold text-white uppercase">{report.mileage_source}</p>
            </Panel>
            <Panel className="p-4">
              <span className="text-label text-steel-300 uppercase">Fleet miles</span>
              <p className="mt-1 font-display text-lg font-extrabold text-white">{Number(report.fleet_miles).toLocaleString()}</p>
            </Panel>
            <Panel className="p-4">
              <span className="text-label text-steel-300 uppercase">Fleet MPG</span>
              <p className="mt-1 font-display text-lg font-extrabold text-white">{Number(report.mpg).toFixed(2)}</p>
            </Panel>
            <Panel className="p-4">
              <span className="text-label text-steel-300 uppercase">Net tax</span>
          ...
          <Panel className="overflow-x-auto mb-4">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.jurisdiction} className="border-b border-white/5">
                    <td className="px-4 py-2.5 font-bold text-white">{row.jurisdiction}</td>
                    <td className="px-4 py-2.5 text-right text-steel-100">{row.miles.toLocaleString()}</td>
                    <td className="px-4 py-2.5 text-right text-steel-100">{row.taxableGallons.toFixed(3)}</td>
                    <td className="px-4 py-2.5 text-right text-steel-100">{row.taxPaidGallons.toFixed(3)}</td>
                    <td className="px-4 py-2.5 text-right text-steel-100">{Number(row.rate).toFixed(4)}</td>
                    <td className="px-4 py-2.5 text-right text-steel-100">{row.surchargeRate ? Number(row.surchargeRate).toFixed(4) : "—"}</td>
          ...
          <p className="text-body-xs text-steel-400 mb-4">
        ...
        <Panel className="p-6 mb-4 text-center">
          <p className="text-white font-semibold">No report computed for {quarter} yet.</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <Panel className="p-4">
          ...
          {rates.length === 0 ? (
            <p className="text-body-sm text-steel-300">None yet — paste from the quarterly iftach.org matrix.</p>
          ) : (
            <div className="flex flex-wrap gap-1.5">
              {rates.map((rate) => (
                <span key={rate.jurisdiction} className="rounded-full border border-white/15 bg-white/5 px-2.5 py-1 text-[11px] font-semibold text-steel-100">
