  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <Panel className="divide-y divide-white/5 max-h-[70vh] overflow-y-auto">
          {expenses.length === 0 ? (
            <p className="p-5 text-body-sm text-steel-300">No expenses yet.</p>
          ) : (
              ...
              <div key={expense.id} className="flex items-center justify-between gap-2 p-3.5 text-sm">
                <div className="min-w-0">
                  <p className="font-semibold text-white capitalize">
                    {expense.category}
                    {expense.memo ? <span className="font-normal text-steel-200"> — {expense.memo}</span> : null}
                  </p>
                ...
                <span className="font-display font-extrabold text-gold shrink-0">{fmtCentsExact(expense.amount_cents)}</span>
              </div>
            ))
          )}
        </Panel>
