    ...
    <div className="space-y-4">
      ...
      {weatherAlerts.length > 0 ? (
        <section className="rounded-2xl border border-gold/40 bg-gold/[0.08] p-4">
          ...
          {weatherAlerts.slice(0, 2).map((alert, i) => (
            <p key={i} className="mt-1 text-body-sm text-fg-2">
              <span className="font-semibold text-fg">{alert.event}:</span> {alert.headline}
            </p>
          ))}
        </section>
      ) : null}
      ...
      {loads.length === 0 ? (
        <section className="rounded-2xl border border-white/10 bg-navy-800/80 p-6 text-center">
          <p className="font-display text-lg font-extrabold text-fg">No active load</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-fg-3">
          ...
          {pay ? (
            ...
            <p className="text-body-xs text-fg-3">No settlements yet</p>
          )}
        </Link>
        <div className="rounded-2xl border border-white/10 bg-navy-800/80 p-4">
          <p className="text-[11px] font-bold uppercase tracking-wider text-fg-3">My cards</p>
          <div className="mt-1.5 space-y-1.5">
            <p className="flex items-center justify-between gap-1 text-body-xs text-fg-2">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 text-center">
            {[
              ...
              <div key={clock.label} className="rounded-xl bg-white/[0.04] py-2">
                ...
                <p className="text-[10px] font-bold uppercase tracking-wider text-fg-3">{clock.label}</p>
              </div>
            ))}
            <p className="col-span-3 text-[10px] text-fg-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <Link
          ...
          className="flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-xl border border-white/15 px-1 font-display text-[11px] font-bold uppercase tracking-[0.04em] text-fg-2 hover:bg-white/5"
        >
