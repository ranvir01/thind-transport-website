  ...
  try {
    ...
    const message =
      err instanceof StreamDropped
        ? "I lost my connection mid-turn, but the work continues on the server. Reopen this chat from the sidebar in a minute or two to see the reply."
    ...
    for await (const event of stream) {
      ...
      switch (event.type) {
        ...
        case "session.status_idle": {
          ...
          await thread.post(`The turn stopped early (${stop.type}). Try again.`);
          return { finished: false, replied };
        }
