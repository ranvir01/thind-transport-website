        ...
        <div className="max-w-4xl hero-stagger">
          ...
          <h1 className="text-left text-white drop-shadow-md mb-5">
            ...
            <span className="block text-xl sm:text-2xl md:text-3xl text-steel-200 font-bold mt-3 normal-case tracking-normal leading-snug">
              Home daily, weekly, or every {PAY_RATES.companyDriver.otr.homeTime} &mdash; real dispatch that answers, {EQUIPMENT.short}, zero forced loads.
            </span>
          </h1>
          ...
          <p className="text-lg md:text-xl text-steel-200 max-w-2xl mb-8 leading-relaxed">
            ...
            {`Owner-operators keep ${PAY_RATES.ownerOperator.commission} of the gross. Company drivers run ${EQUIPMENT.short} at ${PAY_RATES.companyDriver.local.perMile}/mile with weekly pay. ${new Date().getFullYear() - COMPANY_INFO.founded} years out of ${COMPANY_INFO.location} — you drive, we handle the rest.`}
          </p>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-4">
            ...
            <a
              ...
              className="inline-flex min-h-[48px] items-center justify-center gap-2 px-5 py-3.5 text-steel-200 hover:text-orange font-semibold text-sm md:text-base transition-colors"
            >
          ...
          <p className="mb-8 flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-steel-300">
            {/* prefetch=false: this sits in the first viewport, and prefetching
                /pay-rates from here grew the homepage's cold-load JS by 14KB —
                the js-budget ratchet says routes shrink, never grow. */}
            <Link href="/pay-rates" prefetch={false} className="hover:text-orange underline-offset-4 hover:underline">
              See what you&apos;d earn
            </Link>
            <span aria-hidden className="text-steel-600">
              ·
            </span>
            <a
              href={`sms:${COMPANY_INFO.phoneFormatted}?body=${encodeURIComponent("Hi, I'm interested in driving for Thind Transport.")}`}
              className="inline-flex items-center gap-1.5 hover:text-orange"
            >
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-4 rounded-fleet-lg border border-steel-700/50 bg-navy-900/55 px-5 py-4 backdrop-blur-sm sm:gap-x-8 sm:px-7 md:divide-x md:divide-steel-700/50">
            {[
              ...
              { value: PAY_RATES.companyDriver.local.perMile, label: "Company / mile", tone: "text-gold" },
