...
export function NotificationsBell({ direction = "down" }: { direction?: "down" | "up" }) {
  ...
  const toggle = async () => {
    ...
    if (unread > 0) {
      ...
      setUnread(0)
      // The mark-as-read POST must land before the refresh GET, or the
      // GET wins the race and restores the stale unread count.
      try {
        await fetch("/api/hub/notifications", { method: "POST" })
      } catch {
        /* offline — badge stays cleared; the next poll re-syncs */
      }
    }
    refresh()
  }
