...
export function DvirForm({
  ...
  const allOk = () => Object.fromEntries(checklistTemplate.map((c) => [c.key, true]))
  const [checks, setChecks] = useState<Record<string, boolean>>(allOk)
  ...
  // Bumped after a queued submit to remount the pad — its canvas ink can't be
  // erased from outside otherwise.
