...
export function DvirForm({
  ...
  // Bumped to remount the SignaturePad on the queued path — the pad has no
  // imperative clear, and stale ink over a null signature reads as signed.
  ...
  const submit = () =>
    startTransition(async () => {
      ...
      if ("queued" in result) {
        ...
        // same as the load card's queued path. Reset to a fresh form like
        // TimeOffForm: a still-signed form invites a second tap that would
        // queue a duplicate inspection.
