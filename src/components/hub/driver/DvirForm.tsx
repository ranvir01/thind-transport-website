...
export function DvirForm({
  ...
  // Bumped after a queued submit: SignaturePad keeps its ink on an internal
  // canvas with no reset prop, so a remount is the only way to blank it.
  ...
  const submit = () =>
    startTransition(async () => {
      ...
      if ("queued" in result) {
        ...
        // same as the load card's queued path. Reset the form so a driver who
        // doubts the toast can't sign and queue the same inspection twice
        // (TimeOffForm/AdvanceRequestForm clear on queue the same way).
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-white/10 bg-navy-800/80 p-4">
        ...
        <SignaturePad key={padEpoch} onChange={setSignature} height={110} variant="dark" />
        <button
          onClick={submit}
