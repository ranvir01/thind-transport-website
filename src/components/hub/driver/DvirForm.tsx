...
export function DvirForm({
  ...
  // Bumped after a queued submit to remount the SignaturePad — clearing the
  // signature state alone would leave the old ink drawn on the canvas.
  ...
  const submit = () =>
    startTransition(async () => {
      ...
      if ("queued" in result) {
        ...
        // same as the load card's queued path. Reset the form (like TimeOffForm's
        // queued path): a still-signed form with a live button invites a driver
        // who doubts the toast to queue a duplicate inspection.
