    ...
    <>
      <header className="fixed top-0 inset-x-0 z-40 flex h-14 items-center justify-between border-b border-white/10 bg-navy-900/95 px-4 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          <span className="text-body-sm text-steel-200">Hey, {firstName}</span>
          <NotificationsBell />
        </div>
      </header>

      <nav className="fixed bottom-0 inset-x-0 z-40 border-t border-white/10 bg-navy-900/98 backdrop-blur-sm pb-[env(safe-area-inset-bottom)]">
          ...
          {TABS.map((tab) => (
            <Link
              ...
              className={cn(
                ...
                isActive(pathname, tab.href) ? "text-gold" : "text-steel-300 hover:text-white"
              )}
            >
