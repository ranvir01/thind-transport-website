...
import { CloudOff, Loader2, MapPin, ShieldAlert } from "lucide-react"
  ...
  const submit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if ("queued" in result) {
        ...
        // doubtful driver to queue the same report twice.
  ...
  if (queuedOffline) {
    return (
      <div className="rounded-2xl border border-white/10 bg-navy-800/80 p-5 text-center space-y-2">
        ...
        <p className="text-base font-bold text-white">Report saved on your phone</p>
        <p className="text-body-sm text-steel-200">
          It sends to the office automatically when you&apos;re back in signal — no need to file it
          again. Message dispatch any photos from the scene once you&apos;re connected.
        </p>
      </div>
    )
  }
