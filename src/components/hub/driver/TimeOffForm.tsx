...
export function CancelTimeOffButton({ id }: { id: string }) {
  ...
  return (
    <button
      onClick={() =>
        startTransition(async () => {
          // Deliberately NOT queued offline: a stale queued cancel could race
          // an office approval. Tell the truth and let the driver retry.
          try {
            ...
            toast.error(
              isOfflineError(err)
                ? "No signal — try cancelling again when you're back in coverage"
                : "Could not cancel"
            )
          }
        })
