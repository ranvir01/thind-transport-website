...
interface TruckstopFreightSearchProps {
...
function laneLabel(posting: TruckstopLoadPosting): string {
...
const EQUIPMENT_TO_TRUCKSTOP: Record<string, string> = {
  dry_van: "Van",
  reefer: "Reefer",
  flatbed: "Flatbed",
}

export function TruckstopFreightSearch({
  customers,
  canBook,
  truckstopConnected,
}: TruckstopFreightSearchProps) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [pending, startTransition] = useTransition()
  const [postings, setPostings] = useState<TruckstopLoadPosting[]>([])
  ...
  const search = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await searchTruckstopFreightAction({
        ...
        equipment: EQUIPMENT_TO_TRUCKSTOP[criteria.equipment] ?? criteria.equipment,
  ...
  const book = (posting: TruckstopLoadPosting) => {
    ...
    startTransition(async () => {
      const result = await bookTruckstopPostingAction(customerId, posting.raw)
      setBookingId(null)
      if (result.ok && result.id) {
        toast.success("Load booked from Truckstop posting")
  ...
  return (
    <Panel
      className="mb-4"
      title="Truckstop.com freight search"
      action={
        <button
          type="button"
        ...
        >
          ...
          {open ? "Hide" : "Search Truckstop"}
        </button>
      }
    >
      {!truckstopConnected ? (
        <p className="text-sm text-fg-2">
          Connect Truckstop.com under{" "}
        ...
        <div className="space-y-4">
          <form onSubmit={search} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            <div className="sm:col-span-2">
              <label className={labelCls} htmlFor="truckstop-origin-city">
                Origin city
              </label>
              <input
                id="truckstop-origin-city"
                className={fieldCls}
            ...
            <div>
              <label className={labelCls} htmlFor="truckstop-origin-state">
                Origin state
              </label>
              <select
                id="truckstop-origin-state"
                className={fieldCls}
            ...
            <div>
              <label className={labelCls} htmlFor="truckstop-dest-state">
                Dest state
              </label>
              <select
                id="truckstop-dest-state"
                className={fieldCls}
            ...
            <div>
              <label className={labelCls} htmlFor="truckstop-equipment">
                Equipment
              </label>
              <select
                id="truckstop-equipment"
                className={fieldCls}
            ...
            <div>
              <label className={labelCls} htmlFor="truckstop-radius">
                Radius (mi)
              </label>
              <input
                id="truckstop-radius"
        ...
        <p className="text-sm text-fg-2">
          Search Truckstop.com freight by lane and book a posting straight onto your load board.
        </p>
      )}
    </Panel>
  )
