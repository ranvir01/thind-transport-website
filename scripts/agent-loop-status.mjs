...
/**
 * Strict parse of `agent-branch-inventory.mjs --json` output. Throws on
 * malformed/truncated JSON or a missing `pending` array instead of coercing
 * to 0 — a silent 0 here once masked 200+ pending branches when the
 * inventory's output was cut off mid-write.
 */
export function parsePendingCount(raw) {
  const parsed = JSON.parse(raw)
  if (!Array.isArray(parsed?.pending)) {
    throw new Error("inventory JSON has no 'pending' array — output truncated or format changed")
  }
...
function main() {
  ...
  try {
    ...
    pendingFailure = err instanceof Error ? err.message : String(err)
  }
  if (pendingCount === null) {
    console.log(`\nPending claude/* branches (not on main): UNKNOWN — inventory failed: ${pendingFailure}`)
    console.log("  Do NOT read this as 0. Run: npm run agent:branches")
  } else {
