...
/**
 * Parse the inventory's --json payload into a pending-branch count.
 * Returns null (never 0) for unparseable or shape-mismatched output, so a
 * truncated pipe or a renamed key reads as UNKNOWN instead of a false all-clear.
 */
...
function main() {
  ...
  const pendingCount = readPendingCount()
  if (pendingCount === null) {
    console.log("\nPending claude/* branches (not on main): UNKNOWN — inventory --json output unreadable")
