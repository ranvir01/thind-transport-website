 ...
 * Returns null (never 0) when the output is unusable — truncated JSON, or a
 * shape without a pending array. Regression: a truncated pipe once parsed as
 * an error that was swallowed to 0, so agent:status claimed "Pending: 0"
 * while agent:branches listed hundreds.
 */
...
function main() {
  ...
  try {
    ...
    console.log(`\nWARNING: agent-branch-inventory.mjs --json failed: ${err?.message?.split("\n")[0] ?? err}`)
  }
  if (pendingCount === null) {
    console.log("\nPending claude/* branches (not on main): UNKNOWN — inventory JSON unusable.")
