...
/** Parse inventory --json output; null (not 0) when the payload is missing or malformed. */
...
/** Commit-level lower bound, independent of the inventory's cherry logic. */
function rawUnmergedClaudeBranchCount() {
  const out = git(`branch -r --list 'origin/claude/*' --no-merged ${MAIN}`)
  return out
    ? out
        .split("\n")
        .map((l) => l.trim())
        .filter((name) => name && name !== INTEGRATOR).length
    : 0
}
...
function main() {
  ...
  try {
    ...
    /* inventory crashed — parsePendingCount("") reports UNKNOWN below */
  }
  const pendingCount = parsePendingCount(pendingOut)
  const rawUnmerged = rawUnmergedClaudeBranchCount()
  if (pendingCount === null) {
    console.log("\nPending claude/* branches (not on main): UNKNOWN — inventory --json failed or was unparseable")
    console.log(`  Commit-level unmerged claude/* branches (git --no-merged): ${rawUnmerged}`)
    console.log("  Run: npm run agent:branches")
  } else {
    console.log(`\nPending claude/* branches (not on main): ${pendingCount}`)
    if (pendingCount === 0 && rawUnmerged > 0) {
      console.log(
        `  MISMATCH: inventory reports 0 pending but ${rawUnmerged} claude/* branch(es) have unmerged commits — inventory output may be truncated or its filter wrong. Run: npm run agent:branches`
      )
    }
    if (pendingCount > 0) {
      console.log("  Run: npm run agent:branches")
