 ...
 * Returns { count } on success, { error } on any failure — never a silent 0.
 * Regression: truncated/unparseable JSON used to be swallowed to 0, so
 * agent:status said "Pending: 0" while agent:branches listed 200+.
 */
export function parsePendingCount(out) {
  try {
    ...
    return { error: `inventory JSON unparseable (${err.message})` }
  }
}
...
function main() {
  ...
  try {
    const pendingOut = execSync("node scripts/agent-branch-inventory.mjs --json", {
      encoding: "utf-8",
      cwd: process.cwd(),
      maxBuffer: 64 * 1024 * 1024, // 272 branches ≈ 200KB today; leave headroom
    })
