...
/**
 * Strictly parse `agent-branch-inventory.mjs --json` output.
 * Returns { count } on success, { error } otherwise — never a silent 0.
 * Regression: truncated child stdout used to be swallowed to `pending: 0`,
 * so agent:status said "0 pending" while agent:branches listed 200+.
 */
export function parsePendingCount(jsonText) {
  ...
  try {
    ...
    return { error: `inventory --json output unparseable (${err.message})` }
  }
  if (!parsed || !Array.isArray(parsed.pending)) {
    return { error: "inventory --json output has no pending[] array" }
  }
...
function main() {
  ...
  try {
    const pendingOut = execSync("node scripts/agent-branch-inventory.mjs --json", {
      encoding: "utf-8",
      cwd: process.cwd(),
      maxBuffer: 64 * 1024 * 1024, // 200+ session branches already produce >150KB of JSON
    })
    pending = parsePendingCount(pendingOut)
  } catch (err) {
    pending = { error: `inventory run failed (${err.message})` }
  }
  if (pending.error) {
    console.log(`\nPending claude/* branches (not on main): UNKNOWN — ${pending.error}`)
    console.log("  Do not trust 0 here; run: npm run agent:branches")
  } else {
