 ...
 * Returns null (never 0) when the payload is missing or unparseable —
 * truncated JSON must read as "unknown", not "all clear".
 */
...
function main() {
  ...
  try {
    ...
    /* inventory crashed — parsePendingCount(null) reports it below */
  }
  const pendingCount = parsePendingCount(pendingOut)
  if (pendingCount === null) {
    console.log("\nPending claude/* branches (not on main): UNKNOWN — inventory output unreadable")
