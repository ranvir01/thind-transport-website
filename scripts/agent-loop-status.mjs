 ...
 * Returns { count, error } — count is null when the payload is unusable so
 * callers can surface UNKNOWN instead of a silent (and wrong) 0. Truncated
 * JSON (the historic 64KiB pipe bug) lands in the error path, not at zero.
 */
export function parsePendingCount(out) {
  ...
  try {
    ...
    return { count: null, error: `inventory JSON unparseable (${err.message}) — output may be truncated` }
  }
...
function main() {
  ...
  try {
    ...
    inventoryError = `inventory script failed (${err.message})`
  }
  const { count: pendingCount, error: parseError } = inventoryError
    ? { count: null, error: inventoryError }
    : parsePendingCount(pendingOut)
  if (pendingCount === null) {
    console.log(`\nPending claude/* branches (not on main): UNKNOWN — ${parseError}`)
    console.log("  Run: npm run agent:branches for the real list")
  } else {
