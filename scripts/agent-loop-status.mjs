...
/**
 * Parse agent-branch-inventory --json output into a pending count.
 * Returns null (never 0) when the output is missing or unparseable —
 * reporting "0 pending" for a truncated/failed inventory run hid 200+
 * unpicked branches from the integrator.
 */
...
function main() {
  ...
  if (pendingCount === null) {
    console.log(
      "\nPending claude/* branches (not on main): UNKNOWN — inventory --json failed or was unparseable; run npm run agent:branches directly"
    )
  } else {
