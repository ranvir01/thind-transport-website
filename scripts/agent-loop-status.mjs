 ...
 * never a silent 0. A truncated pipe (the original bug: the inventory
 * called process.exit before async stdout flushed) shows up here as a
 * JSON parse error instead of masquerading as "0 pending".
 */
export function parsePendingCount(out) {
  ...
  try {
    ...
    return { count: null, error: `inventory JSON unreadable (${err.message})` }
  }
  if (!Array.isArray(payload?.pending)) {
    return { count: null, error: "inventory JSON has no pending[] array" }
  }
  return { count: payload.pending.length }
}
...
function main() {
  ...
  let pending = { count: null, error: "inventory did not run" }
  try {
    ...
    pending = { count: null, error: `inventory failed to run (${err.message?.split("\n")[0]})` }
  }
