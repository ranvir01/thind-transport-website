...
/** A tiny one-page placeholder PDF so local "email the packet" has real bytes to attach. */
async function writePlaceholderPdf(fileName, title) {
  const pdf = await PDFDocument.create()
  const page = pdf.addPage([612, 400])
  const font = await pdf.embedFont(StandardFonts.Helvetica)
  page.drawText(title, { x: 54, y: 340, size: 16, font, color: rgb(0.1, 0.12, 0.16) })
  page.drawText("Demo placeholder generated by npm run seed:demo.", {
    x: 54, y: 310, size: 10, font, color: rgb(0.4, 0.4, 0.4),
  })
...
async function main() {
  ...
  // Carrier packet rows need real bytes on disk — "Email the packet" reads them back via
  // readDocumentBytes(), and a row with no backing file silently reports "Nothing to send yet".
  await writePlaceholderPdf("thind-w9.pdf", "Form W-9 — Thind Transport LLC")
  await writePlaceholderPdf("thind-coi.pdf", "Certificate of Insurance — Thind Transport LLC")
  await writePlaceholderPdf("thind-authority.pdf", "Authority Letter — Thind Transport LLC")
  await q(
