...
async function main() {
  ...
  await clickByText(fresh, "Continue") // → Branding (FMCSA verify is optional; skip the network call)
  await clickByText(fresh, "Skip for now") // no accent picked → Driver pay
  await shot(fresh, "01b-pay-step")
  await clickByText(fresh, "Continue") // pay defaults prefilled → Your account
