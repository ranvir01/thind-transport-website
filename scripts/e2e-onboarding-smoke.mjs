...
import { BASE, sleep, waitForText, login, makeShot } from "./e2e-lib.mjs"
...
async function main() {
  ...
  const wizardNext = async (label) => {
    await fresh.$$eval(
      "button",
      (btns, want) => {
        const btn = btns.find((b) => b.textContent.includes(want))
        if (!btn) throw new Error(`wizard button "${want}" not found`)
        btn.click()
      },
      label
    )
  ...
  await wizardNext("Skip for now") // no accent → Driver pay (prefilled defaults)
