...
async function main() {
  ...
  await fresh.waitForSelector('[role="radiogroup"]')
  await clickByText(fresh, "Skip for now") // → Driver pay (prefilled defaults)
  await fresh.waitForSelector("#su-permile")
  await clickByText(fresh, "Continue") // → Your account
