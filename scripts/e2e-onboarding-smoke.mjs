...
async function main() {
  ...
  await waitForText(fresh, "accent color") // step 2: branding
  await clickByText(fresh, "Skip for now")
  await waitForText(fresh, "How do you pay drivers") // step 3: pay, prefilled defaults
  await clickByText(fresh, "Continue")
  await fresh.waitForSelector("#su-owner", { timeout: 8000 }) // step 4: account
