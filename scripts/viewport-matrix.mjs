...
for (const mode of ["light", "dark"]) {
  ...
  /**
   * Emulate a device's safe-area insets (the iPhone home indicator). Chromium
   * reports zero unless overridden, so `env(safe-area-inset-bottom)` — which
   * both the tab bar and the content clearance are written in — evaluates to
   * 0 in every ordinary headless run. Verified working on this Chromium:
   * a `calc(7rem + env(safe-area-inset-bottom))` box measured 112px → 146px
  ...
  const setSafeArea = async (bottom) => {
  ...
  for (const [route, needsAuth] of ROUTES) {
    for (const vp of VIEWPORTS) {
      ...
      // every viewport that follows the safe-area one.
      const insetApplied = await setSafeArea(vp.safeAreaBottom ?? 0)
      if (vp.safeAreaBottom && !insetApplied) {
        overflows.push({ route, vp: vp.name, mode, error: "safe-area emulation unavailable in this Chromium" })
        continue
      }
      try {
        ...
        if (vp.mobile) {
          ...
          barProbes.push({
            route, vp: vp.name, mode, status: probe.status,
  ...
  try {
    ...
    for (const vp of VIEWPORTS.filter((v) => v.name === "iphone" || v.name === "iphone-safe")) {
      ...
      try {
        await driverCdp.send("Emulation.setSafeAreaInsetsOverride", {
          insets: { top: 0, left: 0, bottom: vp.safeAreaBottom ?? 0, right: 0 },
        })
      ...
      barProbes.push({
        route: "/hub/driver", vp: vp.name, mode, status: probe.status,
        barH: probe.barH, clearance: probe.clearance,
      })
      if (probe.found.length > 0) {
        overflows.push({
          route: "/hub/driver", vp: vp.name, mode,
          buried: probe.found.map((b) => b.txt).join(" | "),
        })
      }
    }
  } catch (error) {
    overflows.push({ route: "/hub/driver", vp: "iphone", mode, error: error.message })
  } finally {
...
console.log(`${ROUTES.length * VIEWPORTS.length * 2 + 4} screenshots → ${OUT}/`)
...
// Show the geometry the safe-area pass exists to exercise, so a green run is
// evidence rather than an assertion: clearance must track the inset.
for (const name of ["iphone", "iphone-safe"]) {
