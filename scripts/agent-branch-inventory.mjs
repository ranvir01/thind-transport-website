...
function main() {
  ...
  if (json) {
    // No process.exit() here: with stdout piped, exiting right after a >64KB
    // write truncates the buffered output mid-JSON (agent-loop-status then
    // parsed half an object and reported "0 pending" against 200+ branches).
