...
function main() {
  ...
  if (json) {
    // No process.exit here: exiting right after console.log kills the process
    // before piped stdout drains, truncating the JSON at 64KB for consumers
    // like agent-loop-status.mjs. A normal return lets stdout flush fully.
