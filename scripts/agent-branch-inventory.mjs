...
function main() {
  ...
  if (json) {
    // No process.exit() here: when stdout is a pipe (agent-loop-status runs
    // this via execSync), writes past the pipe buffer are async and exit()
    // drops the tail — the consumer then sees truncated, unparseable JSON.
