...
function main() {
  ...
  if (json) {
    // No process.exit here: exiting before stdout drains truncates piped
    // output at the 64KB pipe buffer, which fed agent-loop-status a broken
    // JSON document (it then reported "Pending: 0" against 200+ branches).
