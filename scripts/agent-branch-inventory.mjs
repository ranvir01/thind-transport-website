...
function main() {
  ...
  if (json) {
    // No process.exit() here: stdout to a pipe flushes asynchronously, so an
    // early exit truncates large JSON mid-write — agent:status then parsed
    // the fragment as "Pending: 0" while 200+ branches were pending.
