...
function main() {
  ...
  if (json) {
    // No process.exit() here: stdout to a pipe drains asynchronously, and
    // exiting truncated large reports mid-write — agent:status then parsed
    // the cut-off JSON as "0 pending" while 200+ branches were pending.
