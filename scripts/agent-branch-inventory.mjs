...
function main() {
  ...
  if (json) {
    // No process.exit() after this write: when stdout is a pipe (execSync in
    // agent-loop-status.mjs) console.log is async, and exiting mid-write
    // truncates the JSON — agent:status then parsed garbage and reported 0.
