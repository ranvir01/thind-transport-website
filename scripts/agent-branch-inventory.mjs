...
function main() {
  ...
  if (json) {
    // No process.exit() here: console.log to a pipe is async, and exiting
    // immediately truncates large JSON mid-stream (agent:status read a cut-off
    // document and reported 0 pending while 200+ branches were listed).
