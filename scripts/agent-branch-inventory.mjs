...
function main() {
  ...
  if (json) {
    // No process.exit() here: stdout to a pipe flushes asynchronously, so
    // exiting right after a large console.log truncates the JSON mid-write
    // (agent-loop-status.mjs then read a broken document and reported 0).
