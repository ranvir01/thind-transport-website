...
function main() {
  ...
  if (json) {
    // No process.exit() here: exiting while a large console.log is still
    // flushing to a pipe truncates the JSON mid-stream (agent-loop-status
    // then parsed the fragment as "0 pending" with 200+ branches waiting).
