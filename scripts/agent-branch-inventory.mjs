...
function main() {
  ...
  if (json) {
    // No process.exit() here: with stdout on a pipe (execSync in
    // agent-loop-status.mjs) console.log writes async, and exiting kills the
    // process mid-flush — consumers got truncated JSON for 200+ branches.
