...
function main() {
  ...
  if (json) {
    // No process.exit() here: exiting while stdout is a pipe truncates large
    // payloads mid-write (hundreds of pending branches ≫ the 64KB pipe buffer),
    // which downstream JSON.parse callers see as corrupt output.
