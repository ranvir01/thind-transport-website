...
function main() {
  ...
  if (json) {
    // No process.exit() here: stdout to a pipe is async, and exiting right
    // after console.log truncates large payloads mid-flush (agent:status then
    // parsed the torso and reported 0 pending against 300+ real branches).
