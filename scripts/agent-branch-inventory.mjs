...
function main() {
  ...
  if (json) {
    // No process.exit here: exiting immediately after a large console.log
    // drops the unflushed tail when stdout is a pipe (execSync in
    // agent-loop-status), which truncated the JSON mid-document.
