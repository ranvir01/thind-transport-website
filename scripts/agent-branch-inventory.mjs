...
function main() {
  ...
  if (json) {
    // No process.exit() here: stdout writes to a pipe are async, so exiting
    // immediately truncates large JSON mid-string for callers like
    // agent-loop-status.mjs (execSync). Fall through and let node exit 0
