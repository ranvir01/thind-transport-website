...
function main() {
  ...
  if (json) {
    // No process.exit() here: piped stdout flushes asynchronously, and exiting
    // early truncates the JSON at the 64 KiB pipe buffer (agent:status then
    // parsed the fragment as "0 pending"). Returning lets Node drain stdout.
