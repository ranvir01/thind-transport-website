...
async function main() {
  ...
  // seed:demo does not truncate hub.api_credentials, so a smoke run that died
  // mid-way leaves the card connected — restore the clean start via the UI.
  if (!/not connected/i.test(await cardText(page))) {
    console.log("   (previous run left the card connected — disconnecting first)")
  ...
  // Disconnect is a two-step destructive confirm: "Disconnect" arms it,
  // "Disconnect it" performs it.
  await clickInCard(page, "Disconnect")
  await sleep(300)
  await clickInCard(page, "Disconnect it")
  await waitForText(page, "the CSV import path keeps working")
