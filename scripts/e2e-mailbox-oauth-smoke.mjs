...
async function main() {
  ...
  // Self-heal: seed-demo never truncates hub.api_credentials, so a prior run
  // that died between Connect and Disconnect leaves the mailbox connected and
  // would fail the "starts not connected" check below. Restore the clean
  // state through the same UI path the run under test exercises.
  if (!/not connected/i.test(await cardText(page))) {
    console.log("   (stale connection from a dead prior run — disconnecting first)")
  ...
  // Disconnect is confirm-first (1daecb5): the first click arms the
  // confirmation, the red "Disconnect it" actually disconnects.
  await clickInCard(page, "Disconnect")
  await sleep(300)
  await clickInCard(page, "Disconnect it")
  await waitForText(page, "the CSV import path keeps working")
