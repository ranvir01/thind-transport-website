...
8. **Fresh containers ship Postgres running but with no role or database.** `service postgresql start` only gets you the server plus `postgres`/`template0`/`template1` — `npm run db:migrate` then fails with "POSTGRES_URL is required" or a connection refusal, not a helpful bootstrap error. Before the first migrate: `sudo -u postgres psql -c "CREATE ROLE appuser WITH LOGIN PASSWORD 'appuser' CREATEDB;"` and `sudo -u postgres psql -c "CREATE DATABASE thind_hub OWNER appuser;"`, then set `POSTGRES_URL=postgresql://appuser:appuser@localhost:5432/thind_hub` in `.env.local`.
