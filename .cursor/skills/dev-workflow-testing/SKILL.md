...
| DB layer | `src/lib/driver-db.ts`, `src/lib/driver-db-postgres.ts`, `src/lib/driver-db-local.ts` (node-postgres pool for a local `POSTGRES_URL`) |
...
7. **`/driver/*` (legacy driver portal) no longer uses `@vercel/postgres`.** It once did (Neon's HTTP-fetch driver, which 500s with `fetch failed / ECONNREFUSED 127.0.0.1:443` against a plain local Postgres); everything now goes through node-postgres pools. `src/lib/driver-db-postgres.ts` routes a local `POSTGRES_URL` (`localhost` / `127.0.0.1`, see `isLocalPostgresUrl`) through `src/lib/driver-db-local.ts` — its own `pg` pool with `search_path=public`, so the unqualified legacy tables never resolve into `hub.*` — and otherwise shares the hub pool. So the same local `POSTGRES_URL` serves `/hub/*` and `/driver/*`; if you still see the `:443` error you are on a checkout older than this change. Leaving `POSTGRES_URL` unset still falls back to local JSON files (per `.env.example`), and `GET /api/setup-db` creates the legacy tables when you want them Postgres-backed locally.
