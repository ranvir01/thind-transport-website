...
8. **`scripts/e2e-*.mjs` use Puppeteer, not Playwright.** Plain `npm install` downloads Puppeteer's own bundled Chromium when the sandbox allows the fetch (works out of the box on this Cloud Agent image — no `PUPPETEER_EXECUTABLE_PATH` needed). Only point Puppeteer at a preinstalled browser (e.g. `PUPPETEER_EXECUTABLE_PATH=/opt/pw-browsers/chromium-<rev>/chrome-linux/chrome`) if that download is blocked and the script fails fast with "Could not find Chrome" / a postinstall network error.
