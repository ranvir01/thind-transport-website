...
8. **`scripts/e2e-*.mjs` use Puppeteer, not Playwright** — `npm install` alone leaves Puppeteer without a bundled Chromium in network-restricted agent sandboxes (its postinstall download fails or is skipped). Rather than fighting that, point Puppeteer at whatever Chromium the sandbox already ships (e.g. Anthropic's Claude Code web/cloud containers preinstall one for Playwright at `/opt/pw-browsers/chromium-<rev>/chrome-linux/chrome`): `export PUPPETEER_EXECUTABLE_PATH=/opt/pw-browsers/chromium-<rev>/chrome-linux/chrome` before running `npm install` (skip the download with `PUPPETEER_SKIP_DOWNLOAD=true npm install`) or any `e2e-*.mjs` script. Verify first with a one-line launch check — a stale/wrong path fails fast with "Browser was not found at the configured executablePath".
